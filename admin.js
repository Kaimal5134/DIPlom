...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('nav-reports').addEventListener('click', (e) => {
        ...
        loadReportCategories(); // загружаем категории для фильтра
    });
    ...
    try {
        ...
        form.innerHTML = `
            ...
            <div style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px;">
                <button type="button" class="btn-details" style="background-color: var(--light-gray); color: var(--dark-gray);" onclick="this.closest('.modal').remove()">Отмена</button>
            ...
            row.innerHTML = `
                ...
                <td>
                    <select class="order-status-select" data-order-id="${order.id}" data-current-state="${order.state_id}">
                        <option value="1" ${order.state_id == 1 ? 'selected' : ''}>Заказ оформлен и оплачен</option>
...
// Загрузка категорий для фильтра в отчёте
...
// Скачивание отчёта в PDF (через fetch для обработок ошибок)
...
// Загрузка категорий для фильтра прайс-листа
...
// Отображение прайс-листа в виде таблицы
